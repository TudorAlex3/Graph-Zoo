...
-}
create_list :: (a -> a) -> [a] -> Int -> [[a]] -> [[a]]
create_list f xs index res = if index == (length xs) then (reverse res) else
                             let first_list = take index xs
                                 element_modify = [f (head (drop index xs))]
                                 last_list = (drop (index+1) xs)
                                 new_list = first_list ++ element_modify ++ last_list
                             in create_list f xs (index+1) (new_list : res)
                             
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = create_list f xs 0 []
